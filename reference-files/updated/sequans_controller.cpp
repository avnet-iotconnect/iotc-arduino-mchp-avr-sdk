...
static inline void printChar(char data, bool print_newline) {
    if (isprint(data)) {
        char buff[2];
        buff[0] = data;
        buff[1] = 0;
...
#if 0
            Log.rawf(F("\n##"));
            for (uint16_t i = 0; i < urc_identifier_buffer_length; i++) {
                char ch = (char) urc_identifier_buffer[i];
                if (ch == '\r' || ch == '\n') {
                    ch = '_';
                }
                char chbuf[2];
...
bool SequansControllerClass::waitForURC(const char* urc_identifier,
    ...
    bool is_action_success = true;

    while (is_action_success && !got_wait_for_urc_callback && !timeout_timer.hasTimedOut()) {
        // We update the CTS here in case the CTS interrupt didn't catch the
        // falling flank
        ctsUpdate();
        ...
        if (action != NULL && action_timer.hasTimedOut()) {
            is_action_success = action();
            if (!is_action_success) {
                Log.warn("waitForURC interrupted!");
            }
            action_timer.reset();
        }
    }
