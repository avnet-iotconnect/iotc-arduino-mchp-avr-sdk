...
static bool is_disconnected_early = false;
...
static void internalOnEarlyDisconnectCallback(char* urc_data) {
    ...
    char value_buffer[5] = {0};

    const bool got_value = SequansController.extractValueFromCommandResponse(
        urc_buffer,
        2,
        value_buffer,
        sizeof(value_buffer),
    ...
    int value = abs(atoi(value_buffer));

    // when the TLS connection fails immediately (host refused to connect perhaps), then we will get a negative value
    if (value > 0) {
        is_disconnected_early = true;
    }
}
...
static void handleEarlyDisconnectHelper(void) {
    SequansController.unregisterCallback(F("SQNSMQTTONCONNECT"));
    is_disconnected_early = false;
...
bool MqttClientClass::begin(const char* client_id,
    ...
    is_disconnected_early = false;
    SequansController.registerCallback(F("SQNSMQTTONCONNECT"),
                                        internalOnEarlyDisconnectCallback);
    ...
    if (is_disconnected_early) {
        handleEarlyDisconnectHelper();
        return false;
    }
    ...
    const auto toggle_led = [] () -> bool {
        LedCtrl.toggle(Led::CON, true);
        if (is_disconnected_early) {
            Log.error(F("Got a disconnect indication"));
        }
        return !is_disconnected_early;
    };

    const auto toggle_led_with_printing = [] () -> bool {
        LedCtrl.toggle(Led::CON, true);
        Log.rawf(F("."));
        if (is_disconnected_early) {
            Log.error(F("Got a disconnect indication"));
    ...
    SequansController.unregisterCallback(F("SQNSMQTTONCONNECT")); // unregister early disconnect detection
