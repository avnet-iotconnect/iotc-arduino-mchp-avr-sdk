...
static ATCA_STATUS write_all_data(void) {
    ATCA_STATUS atca_status;
    if ((atca_status = atcab_write_bytes_zone(
        ATCA_ZONE_DATA,
        DATA_SLOT_NUM,
        0,
        data_cache,
        IOTC_DATA_SLOT_SIZE)) != ATCA_SUCCESS
    ) {
...
static ATCA_STATUS load_ecc608_cache(void) {
    ...
    if (!has_iotconnect_data) {
        ...
        atca_status = iotc_ecc608_write_string_value(IOTC_ECC608_PROV_VER, IOTC_ECC608_PROV_DATA_VERSION);
...
ATCA_STATUS iotc_ecc608_copy_string_value(ecc_data_types data_type, char *buffer, uint16_t buffer_size) {
    DataHeaderUnion* h = (DataHeaderUnion *) data_cache;
    buffer[0] = 0;
    while(h->header.type != EMPTY) {
        if (h->header.type == data_type) {
